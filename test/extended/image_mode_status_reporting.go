...
var _ = g.Describe("[sig-mco][Suite:openshift/machine-config-operator/disruptive][Serial][Disruptive][OCPFeatureGate:ImageModeStatusReporting]", g.Ordered, func() {
	...
	g.It("MachineConfigPool machine counts should transition correctly on an update in a default MCP [apigroup:machineconfiguration.openshift.io]", func() {
		...
		runMachineCountTest(machineConfigClient, oc, "90-master-testfile")
	})
	...
	g.It("MachineConfigPool machine counts should transition when OCB is enabled in a default MCP [apigroup:machineconfiguration.openshift.io]", func() {
		...
		// Run the machine count test for a default MCP when applying a standard MachineConfig
		runMachineCountTest(machineConfigClient, oc, "")
	})
})
...
func runImageModeMCNTest(oc *exutil.CLI, machineConfigClient *machineconfigclient.Clientset, mcpAndMoscName, mcName string, isImageUpdate bool) {
	...
	o.Expect(DisableOCL(mosc)).To(o.Succeed(), "Error cleaning up MOSC `%s`", mosc)
	logger.Infof("OK!\n")
...
func runMachineCountTest(machineConfigClient *machineconfigclient.Clientset, oc *exutil.CLI, mcName string) {
	var mosc *MachineOSConfig = nil

	// Get the current config version of te MCP to track throughout the MC apply or OCB enablement
	mcpName := "master"
	exutil.By("Getting the starting config version of the `master` MCP")
	mcp, err := machineConfigClient.MachineconfigurationV1().MachineConfigPools().Get(context.TODO(), mcpName, metav1.GetOptions{})
	o.Expect(err).NotTo(o.HaveOccurred(), "Error getting current config version of MCP `%v`: %s", mcpName, err)
	mcpConfigVersion := mcp.Spec.Configuration.Name
	logger.Infof("OK!\n")
	...
	if mcName != "" { // Handle a standard MC apply case
		...
		mosc, err = CreateMachineOSConfigUsingExternalOrInternalRegistry(oc.AsAdmin(), MachineConfigNamespace, mcpName, nil)
		defer mosc.CleanupAndDelete()
		o.Expect(err).NotTo(o.HaveOccurred(), "Error creating the MachineOSConfig resource: %s", err)
		logger.Infof("OK!\n")
	}
	...
	validateMCPMachineCountTransitions(machineConfigClient, oc, mcpName, mcpConfigVersion, mosc)

	// Get the current config version of the MCP to track throughout the MC deletion
	exutil.By("Getting the updated config version of the `master` MCP")
	mcp, err = machineConfigClient.MachineconfigurationV1().MachineConfigPools().Get(context.TODO(), mcpName, metav1.GetOptions{})
	o.Expect(err).NotTo(o.HaveOccurred(), "Error getting current config veriosn of MCP `%v`: %s", mcpName, err)
	mcpConfigVersion = mcp.Spec.Configuration.Name
	logger.Infof("OK!\n")
...
//   - Some other error has occured in a step of the function
func validateMCPMachineCountTransitions(machineConfigClient *machineconfigclient.Clientset, oc *exutil.CLI, mcpName, originalRenderedConfig string, mosc *MachineOSConfig) {
	...
	o.Eventually(func() bool {
		...
		return MCPIsUpdatedToNewConfig(machineConfigClient, mcpName, originalRenderedConfig)
...
func mcnAndNodeAnnotationMachineCountsMatch(machineConfigClient *machineconfigclient.Clientset, oc *exutil.CLI, mcpName string, mosc *MachineOSConfig) bool {
	...
	var expectedUpdatedCount int32 = 0
	var expectedDegradedCount int32 = 0
