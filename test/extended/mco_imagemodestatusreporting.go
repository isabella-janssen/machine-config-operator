...
var _ = g.Describe("[sig-mco][Suite:openshift/machine-config-operator/disruptive][Disruptive][OCPFeatureGate:ImageModeStatusReporting]", func() {
	...
	g.It("MachineConfigNode properties should match the associated node properties when OCB is enabled in a custom MachineConfigPool", func() {
		...
		err = ValidateMCNForNodeInPool(oc, clientSet, nodeToTest, "worker")
		...
		err = ValidateMCNForNodeInPool(oc, clientSet, nodeToTest, mcpAndMoscName)
		o.Expect(err).NotTo(o.HaveOccurred(), "Error validating MCN for node `%s`: %s", nodeToTest.Name, err)
		logger.Infof("OK!\n")
		...
		// Get the node in `infra` MCP
		infraNodes, err := helpers.GetNodesByRole(clientSet, mcpAndMoscName)
		o.Expect(err).NotTo(o.HaveOccurred(), "Error getting node from MCP `%s`: %s", mcpAndMoscName, err)
		// o.Expect(len(infraNodes)).To(o.Equal(1), "Did not get the expected one `infra` node, got %s", len(infraNodes))
		nodeForTest := infraNodes[0]
		// Validate the MCN for the `infra` node
		err = ValidateMCNForNodeInPool(oc, clientSet, nodeForTest, mcpAndMoscName)
		o.Expect(err).NotTo(o.HaveOccurred(), "Error validating MCN for node `%s`: %s", nodeForTest.Name, err)
		logger.Infof("OK!\n")
		...
		err = CleanupCustomMCP(oc, clientSet, mcpAndMoscName, nodeForTest.Name)
		o.Expect(err).NotTo(o.HaveOccurred(), "Error validating MCN for node `%s`: %s", nodeForTest.Name, err)
		logger.Infof("OK!\n")

		exutil.By("Validate the test node has correct MCN properties")
		// Validate the MCN for the `infra` node
		err = ValidateMCNForNodeInPool(oc, clientSet, nodeForTest, "worker")
		o.Expect(err).NotTo(o.HaveOccurred(), "Error validating MCN for node `%s`: %s", nodeForTest.Name, err)
		logger.Infof("OK!\n")
...
// `ValidateMCNForNodeInPool` validates the MCN of a provided node by checking the following:
//   - Check that `mcn.Spec.Pool.Name` matches provided `poolName`
//   - Check that `mcn.Name` matches the node name
//   - Check that `mcn.Spec.ConfigVersion.Desired` matches the node desired config version
//   - Check that `nmcn.Status.ConfigVersion.Current` matches the node current config version
...
func ValidateMCNForNodeInPool(oc *exutil.CLI, clientSet *framework.ClientSet, node corev1.Node, poolName string) error {
	...
	nodeCurrentImage := node.Annotations[constants.CurrentImageAnnotationKey]
	nodeDesiredImage := node.Annotations[constants.DesiredImageAnnotationKey]
	...
	if mcn.Status.ConfigImage.DesiredImage != nodeDesiredImage {
		logger.Infof("MCN status desired image `%v` does not match node desired image `%v`.", mcn.Status.ConfigImage.Desired, nodeDesiredImage)
