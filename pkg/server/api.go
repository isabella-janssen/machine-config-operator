...
func (sh *APIHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case reqConfigVer.Equal(*semver.New("3.3.0")):
		converted33, err := commonconfigs.ConvertRawExtIgnitionToV3_3(conf)
		if err != nil {
			w.Header().Set("Content-Length", "0")
			w.WriteHeader(http.StatusInternalServerError)
	...
	case reqConfigVer.Equal(*semver.New("3.2.0")):
		converted32, err := commonconfigs.ConvertRawExtIgnitionToV3_2(conf)
		if err != nil {
			w.Header().Set("Content-Length", "0")
			w.WriteHeader(http.StatusInternalServerError)
			klog.Errorf("couldn't convert config for req: %v, error: %v", cr, err)
	...
	case reqConfigVer.Equal(*semver.New("3.1.0")):
		converted31, err := commonconfigs.ConvertRawExtIgnitionToV3_1(conf)
		if err != nil {
			w.Header().Set("Content-Length", "0")
			w.WriteHeader(http.StatusInternalServerError)
			klog.Errorf("couldn't convert config for req: %v, error: %v", cr, err)
	...
	default:
		...
		converted2, err := commonconfigs.ConvertRawExtIgnitionToV2_2(conf)
		if err != nil {
			w.Header().Set("Content-Length", "0")
			w.WriteHeader(http.StatusInternalServerError)
			klog.Errorf("couldn't convert config for req: %v, error: %v", cr, err)
