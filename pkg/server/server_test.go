...
func TestEncapsulated(t *testing.T) {
	...
	assert.Nil(t, err)
	mcIgnCfg, err := commonconfigs.ParseAndConvertConfig(mc.Spec.Config.Raw)
	if err != nil {
	...
	origMc := mc.DeepCopy()
	origIgnCfg, err := commonconfigs.ParseAndConvertConfig(origMc.Spec.Config.Raw)
	if err != nil {
		t.Errorf("decoding Ignition Config failed: %s", err)
	}
...
func TestBootstrapServer(t *testing.T) {
	...
	resCfg, err := commonconfigs.ParseAndConvertConfig(res.Raw)
	if err != nil {
		t.Fatal(err)
	}
