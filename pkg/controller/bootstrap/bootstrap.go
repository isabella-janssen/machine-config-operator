...
func (b *Bootstrap) Run(destDir string) error {
	...
	if fgHandler.Enabled(features.FeatureGateOSStreams) && !version.IsSCOS() && !version.IsFCOS() {
		...
		if errors.Is(err, osimagestream.ErrorNoOSImageStreamAvailable) {
			...
			// no streams are available and let the cluster deploy with zero available streams.
			// TODO @pablintino: Track with a Jira card
			klog.Warningf("Bootstraping of OSImageStreams ended with no available streams.")
		} else if errors.Is(err, osimagestream.ErrorNoOSImageStreamAvailable) {
