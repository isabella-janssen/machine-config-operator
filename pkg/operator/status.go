...
func (optr *Operator) syncMetrics() error {
	...
	_, err = optr.mcoCmLister.ConfigMaps(commonconsts.MCONamespace).Get(daemonconsts.ImageRegistryDrainOverrideConfigmap)
...
func (optr *Operator) cfeEvalCgroupsV1() (bool, error) {
	...
	nodeClusterConfig, err := optr.nodeClusterLister.Get(commonconsts.ClusterNodeInstanceName)
	if err != nil {
		if apierrors.IsNotFound(err) {
			return false, nil
		}
...
func isMachineConfigPoolConfigurationValid(fg featuregates.FeatureGate, pool *mcfgv1.MachineConfigPool, version, releaseVersion, osURL string, machineConfigGetter func(string) (*mcfgv1.MachineConfig, error)) error {
	...
	for _, mcName := range mcs {
		...
		v, ok := mc.Annotations[commonconsts.GeneratedByControllerVersionAnnotationKey]
	...
	if renderedMC.Spec.OSImageURL != osURL {
		...
		_, ok := renderedMC.Annotations[commonconsts.OSImageURLOverriddenKey]
		if !ok {
	...
	rv, ok := renderedMC.Annotations[commonconsts.ReleaseImageVersionAnnotationKey]
	...
	if !ok {
		return fmt.Errorf("Unable to access annotation %s for %s expected: %s", commonconsts.ReleaseImageVersionAnnotationKey, renderedMC.Name, releaseVersion)
	}
