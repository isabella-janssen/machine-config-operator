...
## ⚠️ Important: ValidatingAdmissionPolicy Limitations

**ValidatingAdmissionPolicy cannot fully implement the requested validation** due to CEL limitations in Kubernetes. It cannot access other cluster resources like nodes or machines.

**For the full validation requirements, you MUST use the ValidatingAdmissionWebhook approach.**
...
- **Cons**: **MAJOR LIMITATION**: Cannot access other cluster resources (nodes, machines)
- **Current capabilities**: Only blocks deletion of default MCPs (master/worker) 
- **Cannot implement**: The full requirements (checking active machines/nodes)
- **Best for**: Basic protection against deleting default pools only
...
### Protection Levels by Pool Type:

**Default System Pools** (protected from deletion):
- ❌ `master` pool - **Blocked** by ValidatingAdmissionPolicy (required for cluster)
- ❌ `worker` pool - **Blocked** by ValidatingAdmissionPolicy (required for cluster)

**Custom MCPs** (subject to validation):  
- ⚠️ `custom-role` pool - **Full validation** by ValidatingAdmissionWebhook (recommended)
- ⚠️ `custom-role` pool - **Basic warning only** by ValidatingAdmissionPolicy (limited)
...
### Option 1: ValidatingAdmissionPolicy (Limited - Default MCPs Only)

⚠️ **This only blocks deletion of default MCPs (master/worker), not the full validation requested.**
...
**What this gives you:**
- ❌ Blocks deletion of `master` and `worker` pools
- ⚠️ Shows warning for custom MCPs but **does not block them**  
- ❌ **Cannot check for active machines or targeting nodes**

### Option 2: ValidatingAdmissionWebhook (Full Implementation - Recommended)

✅ **This implements the complete validation including active machines and targeting nodes.**
